---
title: "Switch"
description: "Learn how to use the Switch component in your React Native applications with Galio Framework."
keywords: [
  "Switch",
  "Switch - Documentation"
]
source_url: "/docs/components/switch"
---

import CarbonAds from '@site/src/components/CarbonAds';

# React Native Switch Component

This component was built on top of React Native's Switch component and so it inherits all the props the native component has. This is a controlled component which requires the onValueChange prop so you can update the value state.

<CarbonAds serve="CE7IPK3L" placement="galioio" />

![Switch](/img/docs/switch/switch.png)

### Usage

Imports:

```jsx
import { Switch } from 'galio-framework';
```

Simple example (controlled):

```jsx
const [isEnabled, setIsEnabled] = useState(false);

<Switch
  value={isEnabled}
  onValueChange={setIsEnabled}
  color="primary"
  disabled={false}
/>
```

Uncontrolled (legacy):

```jsx
<Switch initialValue={true} />
```

### Props

| Prop                | Type                                                                              | Default                                                                         | Description                                                                                       |
| ------------------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| **value**           | bool                                                                              | false                                                                           | The switch value (controlled mode)                                                                 |
| **onValueChange**   | function                                                                          | null                                                                            | Callback when switch value changes (required in controlled mode)                                  |
| **color**           | oneOfType(['primary', 'theme', 'error', 'warning', 'success', 'info']), string    | `GalioTheme.COLORS.PRIMARY`                                                       | Switch color                                                                                      |
| **disabled**        | bool                                                                              | false                                                                           | Disables the switch                                                                               |
| **initialValue**    | bool                                                                              | false                                                                           | Initial switch value (uncontrolled mode)                                                          |
| **style**           | object                                                                            | null                                                                            | Custom styles                                                                                     |
| **trackColor**      | object                                                                            | null                                                                            | Track colors for on/off states                                                                    |
| **thumbColor**      | string                                                                            | null                                                                            | Thumb color                                                                                       |
| **ios_backgroundColor** | string                                                                        | null                                                                            | iOS background color                                                                              |
| **...Switch.props** | -                                                                                 | -                                                                               | All React Native Switch props are supported                                                       |
